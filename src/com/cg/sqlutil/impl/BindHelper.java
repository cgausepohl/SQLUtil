...
public final class BindHelper {
    ...
    public static PreparedStatement bindBigDecimal(PreparedStatement ps, int pos, BigDecimal val)
            throws SQLException {
        if (val != null)
    ...
    public static PreparedStatement bindInteger(PreparedStatement ps, int pos, Integer val)
            throws SQLException {
        if (val != null)
            ps.setInt(pos, val);
        else
    ...
    public static PreparedStatement bindBoolean(PreparedStatement ps, int pos, Boolean val)
            throws SQLException {
        if (val != null)
    ...
    public static PreparedStatement bindLong(PreparedStatement ps, int pos, Long val)
            throws SQLException {
        if (val != null)
            ps.setLong(pos, val);
        else
    ...
    public static PreparedStatement bindDouble(PreparedStatement ps, int pos, Double val)
            throws SQLException {
        if (val != null)
    ...
    public static PreparedStatement bindFloat(PreparedStatement ps, int pos, Float val)
            throws SQLException {
        if (val != null)
            ps.setFloat(pos, val);
        else
    ...
    public static PreparedStatement bindByteArray(PreparedStatement ps, int pos, byte[] val)
            throws SQLException {
        if (val != null)
            ps.setBytes(pos, val);
        else
    ...
    public static PreparedStatement bindDate(PreparedStatement ps, int pos, java.util.Date val,
            Calendar calendar) throws SQLException {
        return bindSQLDate(ps, pos, val == null ? null : new Date(val.getTime()), calendar);
    }

    public static PreparedStatement bindSQLDate(PreparedStatement ps, int pos, Date val,
            Calendar calendar) throws SQLException {
        if (val != null) {
            if (calendar == null)
                ps.setDate(pos, val);
    ...
    public static PreparedStatement bindSQLTime(PreparedStatement ps, int pos, java.sql.Time val)
            throws SQLException {
        if (val != null)
            ps.setTime(pos, val);
        else
    ...
    public static PreparedStatement bindBlob(PreparedStatement ps, int pos, java.sql.Blob val)
            throws SQLException {
        if (val != null)
            ps.setBlob(pos, val);
        else
    ...
    public static PreparedStatement bindSQLTimestamp(PreparedStatement ps, int pos,
            java.sql.Timestamp val, Calendar calendar) throws SQLException {
        if (val != null) {
            if (calendar == null)
    ...
    private static int[] createBindTypeArray(Object[] bindVars) throws SQLException {
        ...
        for (int i = 0; i < types.length; i++) {
            ...
            if (o instanceof String)
                ...
                t = Types.TIME;
            else if (o instanceof java.util.Date || o instanceof java.sql.Timestamp)
                t = Types.TIMESTAMP;
            else
                throw new SQLException("cannot map opbject to java.sql.Types: Object class="
                        + o.getClass().getName());
            types[i] = t;
        }
        return types;
    ...
    private static void verifyNotMatchingClassesAndBindings(Object[] vals, int[] bindTypes) {
        ...
        if (bindTypes.length != vals.length)
            throw new IllegalArgumentException(
                    "bindVariables and bindTypes must have the same .length");
        for (int i = 0; i < vals.length; i++) {
            ...
            if (o instanceof String) {
                ...
                if (t != Types.NUMERIC && t != Types.DECIMAL)
                    throw new IllegalArgumentException(
                            "BigDecimal must be mapped to Types.{DECIMAL|NUMERIC} (index=" + i
                                    + ")");
            } else if (o instanceof Boolean) {
                if (t != Types.BIT && t != Types.BOOLEAN)
                    throw new IllegalArgumentException(
                            "Boolean must be mapped to Types.{BIT|BOOLEAN} (index=" + i + ")");
            } else if (o instanceof Integer) {
                ...
                if (t != Types.BIGINT)
                    throw new IllegalArgumentException(
                            "Long must be mapped to Types.BIGINT (index=" + i + ")");
            } else if (o instanceof Float) {
                if (t != Types.REAL)
                    throw new IllegalArgumentException(
                            "Float must be mapped to Types.REAL (index=" + i + ")");
            } else if (o instanceof Double) {
                if (t != Types.FLOAT && t != Types.DOUBLE)
                    throw new IllegalArgumentException(
                            "Double must be mapped to Types.{FLOAT|DOUBLE} (index=" + i + ")");
            } else if (o instanceof byte[]) {
                if (t != Types.BINARY && t != Types.VARBINARY && t != Types.LONGVARBINARY)
                    throw new IllegalArgumentException(
                            "byte[] must be mapped to Types.{BINARY|VARBINARY|LONGVARBINARY (index="
                                    + i + ")");
            } else if (o instanceof Date) {
                if (t != Types.DATE)
                    throw new IllegalArgumentException(
                            "Date must be mapped to Types.DATE (index=" + i + ")");
            } else if (o instanceof java.sql.Time) {
                if (t != Types.TIME)
                    throw new IllegalArgumentException(
                            "java.sql.Time must be mapped to Types.TIME (index=" + i + ")");
            } else if (o instanceof java.sql.Timestamp) {
                if (t != Types.TIMESTAMP)
                    throw new IllegalArgumentException(
                            "java.sql.Timestamp must be mapped to Types.TIMESTAMP (index=" + i
                                    + ")");
            } else {
                throw new IllegalArgumentException(
                        "bindVariables[" + i + "], no mapping to java.sql.Types for JAVA-class "
                                + o.getClass().getName() + " (index=" + i + ")");
            }
        }
    }
        ...
        for (int n = 0, pos = 1; n < bindVariables.length; n++, pos++) {
            ...
            case Types.TIMESTAMP_WITH_TIMEZONE: // TODO: mapping correct?
                if (val == null || val instanceof java.sql.Timestamp)
                    ...
                    bindSQLTimestamp(ps, pos,
                            new java.sql.Timestamp(((java.util.Date) val).getTime()), calendar);
                break;
            case Types.BLOB:
