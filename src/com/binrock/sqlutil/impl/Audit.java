...
public class Audit implements AuditInterface {
    ...
    private String insertStmtSQL = INSERT_STMT_SQL_DEFAULT,
            insertStmtExecution = INSERT_STMT_EXECUTION_DEFAULT;
    ...
    public void store(UseCurrentThread ownThread, String appId, StoreBindVariables storeBindValues,
        ...
        try {
            ...
            if (sql.getDBProduct() != DBProduct.POSTGRESQL) {
                System.out.println("only postgres supported as audit target. current dbproduct="
                        + sql.getDBProduct());
                return;
            }
            ...
            // Postgres
            sql.executeDDL(
                    "create table if not exists sqlutil_audit_sql (sql_id numeric(18,0) not null primary key,appid varchar(100) not null,created timestamp not null,sql_text text not null)");
            sql.executeDDL(
                    "create table if not exists sqlutil_audit_execution (sql_id numeric(18,0) not null references sqlutil_audit_sql(sql_id),started timestamp not null,duration_ms numeric(18,3),rowsaffected numeric(18), err varchar(1000),bindvalues text)");

            // Set Ids
            int[] bindTypesSQL = { Types.BIGINT, Types.VARCHAR, Types.TIMESTAMP, Types.VARCHAR };
            int[] bindTypesExecution = { Types.BIGINT, Types.TIMESTAMP, Types.REAL, Types.INTEGER,
            ...
            if (UseBatchInserts.NO == batched) {
                ...
                while (sqls.hasMoreElements()) {
                    ...
                    for (int i = 0; i < recs.size(); i++) {
                        ...
                        Object[] bindVariables = { sqlId, ar.getStarted(), ar.getDurationMs(),
                                ar.getRowsAffected(), err, bindVar };
                ...
                while (sqls.hasMoreElements()) {
                    ...
                    for (AuditRecord ar : recs) {
                        String err = ar.getError() != null ? ar.getError().getMessage() : null;
                        String bindVar = getBindVarValue(ar, storeBindValues);
                        Object[] rowInsertExec = { sqlId, ar.getStarted(), ar.getDurationMs(),
                                ar.getRowsAffected(), err, bindVar };
    ...
    private String getBindVarValue(AuditRecord r, StoreBindVariables store) {
        if (StoreBindVariables.NO == store)
            return null;
        if (sqlPatternsForBindValueMasking == null)
            return null;
        if (sqlPatternsForBindValueMasking.size() == 0)
            return null;
